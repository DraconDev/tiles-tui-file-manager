...
impl EventLoopCtx {
    ...
    /// Handle PreviewRequested: load file content and display in editor preview.
    /// Spawns an async task to do I/O, then installs the preview state.
    pub fn handle_preview_requested(
        &self,
        pane_idx: usize,
        path: PathBuf,
        app_clone: Arc<Mutex<App>>,
    ) {
        let tx = self.event_tx.clone();
        ...
        tokio::spawn(async move {
...

            {
                ...
                let preview = crate::app::PreviewState {
                    path: path.clone(),
                    content,
                    editor: Some(editor),
                    last_saved: None,
                ...
                if is_git_url
                    || app_guard.core.current_view == crate::app::CurrentView::Editor
                    || app_guard.core.current_view == crate::app::CurrentView::Commit
                {
